#!/bin/bash
#SBATCH --job-name=stability_radiomics_clinical
#SBATCH --output=logs/stability_radiomics_clinical_log_%j.log
#SBATCH --error=logs/stability_radiomics_clinical_log_%j.log
#SBATCH --gres=gpu:1
...
python -m src.stability_logreg --config configs/config_radiomics_clinical.yaml --repeats 50 --outdir outputs/stability_radiomics_clinical
